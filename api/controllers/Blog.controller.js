...
export const getAllBlogs = async (req, res, next) => {
  try {
    // This legacy function assumed an authenticated user; keep behavior but ignore missing user
...
// Public blog listing that never requires authentication
export const getPublicBlogs = async (req, res, next) => {
  try {
    const blog = await Blog.find()
      ...
      .lean()
      .exec();
    console.log(`[getPublicBlogs] Returned ${blog.length} blogs`);
    res.status(200).json({ blog });
  } catch (error) {
    next(handleError(500, error.message));
  }
